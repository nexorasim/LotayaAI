...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen text-white overflow-hidden animated-bg">
      {/* Navigation */}
      ...
      {/* Hero Section */}
      <section className="hero-background relative min-h-screen flex items-center justify-center px-4">
        {/* Animated Particles */}
        ...
        <div className="max-w-6xl mx-auto text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-8 justify-center items-center">
            <Link 
              to="/image-generator" 
              className="hero-btn animated-btn btn-primary inline-flex items-center space-x-3 text-lg px-10 py-4 rounded-xl transform-gpu hover:scale-105 transition-transform duration-300"
            >
            ...
            <Link 
              to="/free-tools" 
              className="hero-btn animated-btn btn-secondary inline-flex items-center space-x-3 text-lg px-10 py-4 rounded-xl transform-gpu hover:scale-105 transition-transform duration-300"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {products.map((product, index) => (
              <Link 
                key={index} 
                to={product.link}
                className="product-card group p-8 rounded-2xl transform-gpu hover:scale-105 hover:-translate-y-2 transition-all duration-300"
                style={{
      ...
      {/* Models Section */}
      <section id="models" className="py-20 px-4 bg-black bg-opacity-20">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-6xl font-orbitron font-bold mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {models.map((model, index) => (
              <div 
                key={index} 
                className="model-card p-6 rounded-xl transform-gpu hover:scale-105 transition-all duration-300"
                style={{
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {effects.map((effect, index) => (
              <div 
                key={index} 
                className="effect-tag px-6 py-3 rounded-full font-semibold hover:bg-white hover:bg-opacity-20 hover:scale-105 transition-all duration-300 transform-gpu cursor-pointer"
                style={{
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {freeTools.map((tool, index) => (
              <Link 
                key={index} 
                to="/free-tools"
                className="tool-card group p-6 rounded-xl hover:bg-white hover:bg-opacity-10 hover:scale-105 transition-all duration-300 transform-gpu"
                style={{
                  background: 'rgba(255, 255, 255, 0.08)',
                  backdropFilter: 'blur(15px)',
                  border: '1px solid rgba(255, 255, 255, 0.15)'
